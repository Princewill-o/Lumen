...
import { ArrowUpRight, ArrowRight, Sun, Moon, Menu, X, MessageSquare, Phone, Calendar, Mail, RefreshCw, FileText, Sparkles, Check, Plus, Minus, Zap } from 'lucide-react'
import { ThemeProvider } from './context/ThemeContext'
import { useTheme } from './context/useTheme'
import PointerEffects, { ScrollEffects } from './components/ui/PointerEffects'
import Portfolio from './components/Portfolio'

const solutions = [
  ['Customer enquiries', 'A great first impression. Every time.', 'Respond to customers across your website, email and messaging channels.', MessageSquare, 'A customer asks about your services. Lumen answers using your business information and collects the details your team needs.'],
  ['AI phone assistants', 'Never miss the conversation.', 'Answer calls, qualify enquiries and help customers book appointments.', Phone, 'An incoming caller explains what they need. Your assistant collects their details and routes the enquiry to the right person.'],
  ['Bookings & appointments', 'Your calendar, taken care of.', 'Arrange appointments and send timely reminders automatically.', Calendar, 'Customers choose an available appointment. The booking is added to your calendar and a reminder is sent before their visit.'],
  ['Email automation', 'An inbox that moves itself.', 'Sort, understand and respond to repetitive emails.', Mail, 'Routine questions get an approved response, while sensitive or unusual requests are flagged for your team.'],
  ['Customer follow-ups', 'Keep opportunities moving.', 'Follow up with potential customers so opportunities don’t disappear.', RefreshCw, 'After a quote is sent, a timely follow-up checks whether the customer needs help and updates your records.'],
  ['Content & social media', 'More presence. Less preparation.', 'Turn your business knowledge into posts, captions and content.', Sparkles, 'Turn a service update into draft social posts, ready for your team to review and publish.'],
  ['Documents & admin', 'Give your team their time back.', 'Extract information and automate repetitive administrative work.', FileText, 'Details from a document are organised into your business systems, with exceptions sent to a person for review.'],
]
const industries = ['Property', 'Construction', 'Trades', 'Professional Services', 'Retail', 'Hospitality', 'Recruitment', 'Marketing', 'Other']
const examples = ['Qualify property enquiries and arrange viewings.', 'Collect project details and organise quote requests.', 'Handle job enquiries while your team is on site.', 'Route client enquiries and simplify appointment scheduling.', 'Answer product questions and organise customer requests.', 'Handle booking enquiries and common guest questions.', 'Collect candidate details and coordinate interviews.', 'Prepare content drafts and organise campaign enquiries.', 'If your business has repetitive work, there’s a good chance Lumen can help.']
const faqs = [ ['What does Lumen actually do?', 'Lumen designs and builds AI-powered systems that automate repetitive business processes. Each system is built around your business and the way your team works.'], ['Do I need to understand AI?', 'No. Tell us what you want to improve and we’ll identify where AI and automation could help.'], ['Can you work with our existing tools?', 'In many cases, yes. We can integrate AI systems with existing business tools and workflows where technically appropriate.'], ['Is Lumen suitable for a small business?', 'Yes. Solutions can be designed around the size and needs of your business.'], ['How much does it cost?', 'Every Lumen system is designed around the business and its requirements. Contact us to discuss your needs and receive a tailored proposal.'], ['How long does it take?', 'This depends on the complexity of the system. We’ll provide an estimated timeline after understanding your requirements.'] ]
function Logo({ large = false }) { const { theme } = useTheme(); return <span className={`${large ? 'hero-logo' : 'brand-logo'} logo-lockup`}><img src="/images/lumen-light-transparent.png" alt="Lumen" width="1254" height="1254"/>{theme === 'dark' && <img className="logo-white-wordmark" src="/images/lumen-light-transparent.png" alt="" aria-hidden="true" width="1254" height="1254"/>}</span> }

function Demo() {
 const [step, setStep] = useState(0); const [value, setValue] = useState(''); const [answers, setAnswers] = useState([])
 const prompts = ['Absolutely. We cover North London. I can collect a few details so the team can prepare your quote. What’s the property postcode?', 'Thank you. What type of renovation are you planning?', 'And what’s your preferred timeframe?']
...
function EnquiryForm({ demo = false, selectedPackage = 'Starter Web Presence (£495 / £175 mo)', onSelectPackage }) {
 ...
 async function submit(e) {
  ...
  const data = Object.fromEntries(formData); data.improvements = formData.getAll('improvements'); data.type = demo ? 'demo' : 'enquiry'
